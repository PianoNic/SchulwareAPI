...
class MobileProxyController:
    ...
    async def get_user_info(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me", "GET"))

    # === Grades ===

    @router.get("/grades", dependencies=[Depends(security)], response_model=list[GradeDto])
    async def get_grades(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/grades", "GET"))
    ...
    async def get_events(
        ...
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/events", "GET", query_params=query_params))
    ...
    async def get_exams(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/exams", "GET"))

    # === Absences ===

    @router.get("/absences", dependencies=[Depends(security)], response_model=list[AbsenceDto])
    async def get_absences(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/absences", "GET"))

    @router.get("/absencenotices", dependencies=[Depends(security)], response_model=list[AbsenceNoticeDto])
    async def get_absence_notices(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/absencenotices", "GET"))

    @router.get("/absencenoticestatus", dependencies=[Depends(security)], response_model=list[AbsenceNoticeStatusDto])
    async def get_absence_notice_status(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "config/lists/absenceNoticeStatus", "GET"))

    @router.get("/absences/confirmed", dependencies=[Depends(security)])
    async def get_absences_confirmed(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/absencesAndLateness/isAlreadyConfirmed", "GET"))

    # === Lateness ===

    @router.get("/lateness", dependencies=[Depends(security)], response_model=list[LatenessDto])
    async def get_lateness(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/lateness", "GET"))

    # === Vacations ===

    @router.get("/vacations", dependencies=[Depends(security)], response_model=list[VacationDto])
    async def get_vacations(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/vacations", "GET"))

    # === Notes ===

    @router.get("/homework", dependencies=[Depends(security)], response_model=list[HomeworkDto])
    async def get_homework(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/notes/homework", "GET"))

    @router.get("/objectives", dependencies=[Depends(security)], response_model=list[ObjectiveDto])
    async def get_objectives(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/notes/objectives", "GET"))

    # === Notifications ===

    @router.get("/notifications", dependencies=[Depends(security)], response_model=list[NotificationDto])
    async def get_notifications(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/notifications/push", "GET"))

    @router.get("/topics", dependencies=[Depends(security)], response_model=list[TopicDto])
    async def get_topics(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "me/notifications/topics", "GET"))

    # === Config ===

    @router.get("/settings", dependencies=[Depends(security)], response_model=list[SettingDto])
    async def get_settings(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "config/settings", "GET"))

    @router.get("/customfields", dependencies=[Depends(security)])
    async def get_custom_fields(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "config/customFields", "GET"))

    @router.get("/filecategories", dependencies=[Depends(security)])
    async def get_file_categories(self, token: str = Depends(get_current_token)):
        return await self.mediator.send(ProxyMobileRestQuery(token, "config/filestoreCategories", "GET"))

    # === Student ID Card ===

    @router.get("/studentidcard/{report_id}", dependencies=[Depends(security)], response_model=StudentIdCardDto)
    async def get_student_id_card(self, report_id: int, token: str = Depends(get_current_token)):
        response = await self.mediator.send(ProxyMobileRestQuery(token, f"me/cockpitReport/{report_id}", "GET"))
