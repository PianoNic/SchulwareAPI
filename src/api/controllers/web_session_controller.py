...
class WebSessionController:
    ...
    async def capture_session(self, request: Request, body: WebSessionRequestDto):
    ...
    async def scrape(self, request: Request, body: WebScrapeRequestDto):
        ...
        """
        return await self.mediator.send(ScrapeWebPageQuery(body))

    @router.post("/validate")
    @shared_limiter.limit("10/minute")
    async def validate(self, request: Request, body: WebScrapeRequestDto):
        """Check if a web session is still valid."""
        return await self.mediator.send(ValidateWebSessionQuery(body))
